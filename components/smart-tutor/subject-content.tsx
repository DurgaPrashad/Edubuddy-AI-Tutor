  ...
  const getTopics = () => {
    if (subject === "mathematics") {
      ...
      if (gradeLevel <= 3) {
        ...
        return [
          { name: "Reading Comprehension", progress:  /> },
        ]
      } else if (gradeLevel <= 6) {
        return [
          { name: "Reading Comprehension", progress: 75, icon: <BookOpen className="h-5 w-5" /> },
          { name: "Vocabulary Building", progress: 70, icon: <BookOpen className="h-5 w-5" /> },
          { name: "Grammar and Usage", progress: 65, icon: <BookOpen className="h-5 w-5" /> },
          { name: "Writing Process", progress: 60, icon: <BookOpen className="h-5 w-5" /> },
          { name: "Literature", progress: 80, icon: <BookOpen className="h-5 w-5" /> },
        ]
      } else {
        return [
